...
!     Spring
  ...
  ! Reserves are assumed to be released when we are in the growing season
    ...
    if (abs(thinFRT - 0.) <= epsilon(thinFRT)) then
  ...
  Subroutine NPP(fTran, BIAS, RANDERR, fLUETERR)
    ...
    if (BIAS == 0) then
      ...
      fLUET  = exp( -0.5*((T - (TOPT + 5.))/TTOL**2)**2. )
    end if
    if (RANDERR == 1) then
      fLUET = fLUET*fLUETERR
    end if
  ...
  Subroutine allocation(FORTYPE,fTran,LAI, BIAS, RANDERR, FRERR)
  integer :: FORTYPE,BIAS, RANDERR
  real :: fTran, LAI, FRERR
    FL = FLMAX * fTran * NEFF                                     ! -
    if (FORTYPE==2) then
       if (LAI>LAIMAX) FL = 0
    end if
