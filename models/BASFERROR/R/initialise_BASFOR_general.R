...
#' @param rs Integer, indicating if the model is restarted ( set rs to 1) or started from scratch ( set rs to 0)
#' @param bias Integer, indicating if the model has a process bias (set bias to 1) or has no process bias ( set bias to 0)
#' @param staterr Integer, indicating if the model has a statistical error (set staterr to 1) or has no stat. error ( set staterr to 0)
...
#' @param sv Vector that contains the state variables 
#' @param er Vector that contains the error variables 
...
#' @export
#' @useDynLib BASFERROR
run_mod_model <- function(rs  = RESTART,
                      ft      = FORTYPE,
  ...
  output <- .Fortran('BASFERROR',rs, ft, statespace,  randerr,bias, p,w,calf,calN,calpT,caltT,n,NOUT,
                      sv, stateers, procerr, matrix(0,n,NOUT),PACKAGE = "BASFERROR") [[17]]
  return(output)
}
...
#' @details This function converts the weather data from a data frame into an R matrix (matrix_weather) which can be passed to BASFOR via the function run_mod_model.
...
#' @return Returns a matrix (matrix_weather) of dimension [length in days, 7] which is used to input weather to BASFOR in the run_mod_model function.
#' @examples
#' year_start     <- as.integer(1900); doy_start <- as.integer(1); NDAYS <- as.integer(40543)
#' df_weather     <- weather_CONIFEROUS_1
#' matrix_weather <- weather_BASFOR( year_start, doy_start, NDAYS, df_weather )
...
#' @return Returns a list containing calendar_fert, calendar_Ndep, calendar_prunT, calendar_thinT, outputNames, outputUnits, NOUT needed by functions run_mod_model and plot_output to run BASFOR and plot outputs.
#' @examples
#' init <- initialise()
#' @export
initialise <- function ()
...
SA <- function( parname_SA = "KAC",
  ...
  for (im in 1:nmult) {
    ...
    list_output[[im]] <- run_mod_model(p=params)
  }
