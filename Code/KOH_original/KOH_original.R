...
source("KOH_original/GetReferenceData_add_noise_KOH_originial.R")
...
LL <- function(pars){
  ...
  fit_GPP = gausspr(x = GPP_complet , y = diffGPP)
  fit_ET = gausspr(x = ET_sampled,  y = diffET)
  ...
  diffGPP <-  abs(fullGPPdata - out[,19] - fit_GPP@fitted)
  diffET <- abs(fullETdata - out[,21] - fit_ET@fitted)
...
out_MCMC <- runMCMC(bayesianSetup, settings = list(iterations = 120, nrChains = 1, Z = out_optim$member$pop, startValue = out_optim$member$pop))


save(out_MCMC, file = "./Results/MCMC_func_KOH_original_add_noise.RData")
